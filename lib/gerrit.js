...
gerrit.clone = function(gerrit_name, project_name, destination_folder, install_hook) {
  return gerrit.config(gerrit_name)
    .then(function(config) {
      ...
      logger.info(["Cloning project %s from %s config into folder %s...", project_name, gerrit_name, destination_folder]);
...
gerrit.up = function(remote, branch, is_draft, topic, hashtag) {
  ...
  if (topic === "auto") {
    ...
    var branch_name = git.branch.name()
    if (!/^review\//.test(branch_name)) {
      topic = branch_name;
    }
  }
  ...
  return requireInRepo()
    ...
    .then(function(remoteConfig) {
      ...
      ref = util.format("refs/%s/%s", type, branch);

      logger.info(["Pushing to %s (%s). Topic: %s, hashtag: %s", remoteConfig.name, ref, topic, hashtag]);
      ...
      if (topic)
        args.push("-o", util.format("topic=%s", topic));

      hashtag.forEach( function (tag) {
        args.push("-o", util.format("t=%s", tag));
      });
  ...
  return requireInRepo()
    ...
    .then(function(removeBranch) {
      ...
      if (topic && !git.branch.exists(topic)) {
        git.branch.create(topic, "FETCH_HEAD");
        git.checkout(topic);
...
gerrit.topic = function(name, upstream, force) {
  if (!upstream) {
    upstream = git.branch.upstream();
  }
  return [
    git.branch.create(name, "HEAD", force),
    git.checkout(name),
